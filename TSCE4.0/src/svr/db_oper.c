...
check_conn(MYSQL** conn)
{
	...
	if (0 == mysql_query(*conn, CHECK_STATEMENT)){
		...
		print_debug("check error: ERROR %u (%s): %s\n", mysql_errno(*conn),
...
//                		}
    ...
    if (strcmp(res_buf, "") == 0)
    {
        print_debug("result is null, str_sql:%s\n", str_sql);
        return NULL;
    }
...
unsigned long operate_res(char* str_sql, MYSQL* conn_ptr)       //插入函数
{
    ...
    if (conn_ptr)
    {
        ...
        {
            print_debug("mysql code is error\n");
            return 0;
        }
