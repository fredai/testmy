...
//        fprintf(stdout,"%s,%llu,%.4lf,", time_buffer, tsc, total_elapsed_time);
        for (i = node; i < num_node; i++) {
            ...
            for (domain = 0; domain < RAPL_NR_DOMAIN; ++domain) {
                if(is_supported_domain(domain)) {
               //     fprintf(stdout, "%.4lf,%.4lf,%.4lf,", power_watt[i][domain], cum_energy_J[i][domain], cum_energy_mWh[i][domain]);
			//		fprintf ( stdout, "%.4lf,%.4lf, %.4lf, %.4lf,", power_watt[i][RAPL_PKG],power_watt[i][RAPL_PP0], power_watt[i][RAPL_PP1],power_watt[i][RAPL_DRAM] );
					printf ("\n");
					fprintf (stdout, "%lu", interval_elapsed_time);
			//		printf("\n");
				//	fprintf ( stdout, "%.4lf,%.4lf, %.4lf, %.4lf,", cum_energy_J[i][RAPL_PKG]/total_elapsed_time,cum_energy_J[i][RAPL_PP0]/total_elapsed_time,\
							cum_energy_J[i][RAPL_PP1]/total_elapsed_time,cum_energy_J[i][RAPL_DRAM]/total_elapsed_time );
                }
            }
        }
        fprintf(stdout, "\n");
    ...
    for (i = node; i < num_node; i++) {
        ...
        //    fprintf(stdout, "Average Processor Power_%d(Watt)=%.4lf\n\n", i, cum_energy_J[i][RAPL_PKG]/total_elapsed_time);
			snprintf ( (mod->info[0]).index_data, LEN_32, "%.2f", cum_energy_J[i][RAPL_PKG]/total_elapsed_time);
        }
        if(is_supported_domain(RAPL_PP0)){
        ...
        //    fprintf(stdout, "Average IA Power_%d(Watt)=%.4lf\n\n", i, cum_energy_J[i][RAPL_PP0]/total_elapsed_time);
			snprintf ( (mod->info[1]).index_data, LEN_32, "%.2f", cum_energy_J[i][RAPL_PP0]/total_elapsed_time);
        }
        if(is_supported_domain(RAPL_PP1)){
        ...
        //    fprintf(stdout, "Average GT Power_%d(Watt)=%.4lf\n\n", i, cum_energy_J[i][RAPL_PP1]/total_elapsed_time);
			snprintf ( (mod->info[2]).index_data, LEN_32, "%.2f", cum_energy_J[i][RAPL_PP1]/total_elapsed_time);
        }
        if(is_supported_domain(RAPL_DRAM)){
        ...
        //    fprintf(stdout, "Average DRAM Power_%d(Watt)=%.4lf\n\n", i, cum_energy_J[i][RAPL_DRAM]/total_elapsed_time);
			snprintf ( (mod->info[3]).index_data, LEN_32, "%.2f", 0.5);
        }
    }
//    read_tsc(&tsc);
		terminate_rapl();
