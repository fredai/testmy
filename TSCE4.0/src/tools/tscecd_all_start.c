...
#define TEYE_CLIENT_START_SUCCESS "%s:success "
#define TEYE_CLIENT_START_FAILED  "%s:failed "
...
//    ret = init_app_svr_config(SVR_CONFIG_PATH, &app_svr_config);
    ...
    if ( (f = fopen(TEMP_START_RESULT_FILE, "a+")) < 0) {
        err_sys("open temp file %s failed", TEMP_START_RESULT_FILE);
        exit(1);
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        printf("%s:\n", node->next->node_name);
        sprintf(cmd, "ssh %s \"cd %s;./tscecd_start\"",
                node->next->node_name, argv[2]);
        ret = system(cmd);
        if (ret != 0) {
            fprintf(f, TEYE_CLIENT_START_FAILED, node->next->node_name);
        } else {
            fprintf(f, TEYE_CLIENT_START_SUCCESS, node->next->node_name);
        }
        node = node->next;
    }
