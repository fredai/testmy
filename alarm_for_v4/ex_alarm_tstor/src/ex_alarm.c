...
int main ( int argc, char * argv [ ] ) {
    ...
    if ( ret < 0 ) {
        fprintf ( stderr, "Test databas error, lack of table AlarmInfo\n" );
        exit ( 5 );
    }
...
//        }
    ...
    fprintf(stdout, "%s", "ex_alarm started");
...
/*    	alarm_item_t * p = & ( alarm_items.alarm_item_head );
    	while ( p -> next ) {
		printf("****p->next->alarm_item==%s****\n", p->next->alarm_item_name);
		alarm_item_node_t * q = & ( p->next->alarm_item_node_head );
		while(q->next) {
			printf("****p->next->alarm_item_node_name==%s******\n", q->next->alarm_item_node_name);
			q = q->next;
		}
		p = p->next;
	}
*/
	MYSQL_ROW record_data;
	MYSQL_RES *results_data;
	char db_buffer [ MAX_SQL_LEN ];
        /* fetch and process data */
	while ( fetch_alarm_data ( connection, results, & alarm_data , &alarm_items) ) {
		
		alarm_item_t * p = & ( alarm_items.alarm_item_head );
		while ( p -> next ) {//alarm_item
			
			strcpy ( alarm_data.item, p->next->alarm_item_name );// fill in alarm_data.item		
			alarm_item_node_t * q = & ( p->next->alarm_item_node_head );
			while(q->next) {//alarm_node
				if(!strcmp(q->next->alarm_item_node_name, alarm_data.node)) {
					// produce sql statement 
					sprintf ( db_buffer, GET_SQL_DATA , p->next->alarm_item_name, alarm_data.id);
					// execute sql statement 
					ret = mysql_query ( connection, db_buffer );
					if ( ret != 0 ) {
						return -2;
					}
					results_data = mysql_store_result ( connection );
					if ( results_data == NULL ) {
						return -3;
					}
					record_data = mysql_fetch_row ( results_data );
					alarm_data.value = strtof ( record_data[0], NULL );
//					printf("file = %s line =%d alarm_data->id = %lld\n ", __FILE__, __LINE__, alarm_data.id);    
//				    	printf("file = %s line =%d alarm_data->datatime = %s\n ", __FILE__, __LINE__, alarm_data.datetime);    
//				    	printf("file = %s line =%d alarm_data->node = %s\n ", __FILE__, __LINE__, alarm_data.node);    
//				    	printf("file = %s line =%d alarm_data->item = %s\n ", __FILE__, __LINE__, alarm_data.item);    
//				    	printf("file = %s line =%d alarm_data->value = %f\n ", __FILE__, __LINE__, alarm_data.value);    
					...
					ex_mysql_free_results ( results_data );
				}
				q = q->next;
			}
			p = p -> next;
        ...
        if ( alarm_node -> timer_start [ ALARM_LEVEL_SERIOUS ] == 0 ) {
            ...
            if ( time ( NULL ) - alarm_node -> timer_start [ ALARM_LEVEL_SERIOUS ] >= con_secs ) {
                ...
                if ( alarm_node -> alarm_flag [ ALARM_LEVEL_SERIOUS ] == HAVE_NOT_ALARM ) {
					...
					//changed by fuchencong
					th_arg -> alarm_value = alarm_data -> value;
                    /* ignore return value */
