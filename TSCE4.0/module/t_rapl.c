...
static struct mod_info rapl_mod_info[] = {
	{"rapl_pkg","\0"},
	{"rapl_pp0","\0"},
	{"rapl_pp1", "\0"},
	{"rapl_dram", "\0"}
...
init_rapl()
{
    ...
    default:
        ...
        perror("RAPL not supported, or machine model %x not recognized.\n processor_signature");
        return MY_ERROR;
    }
            ...
            for (domain = 0; domain < RAPL_NR_DOMAIN; ++domain) {
                if(is_supported_domain(domain)) {
					...
					//fprintf ( stdout, "%.4lf,%.4lf, %.4lf, %.4lf,", power_watt[i][RAPL_PKG],power_watt[i][RAPL_PP0], power_watt[i][RAPL_PP1],power_watt[i][RAPL_DRAM] );
							snprintf ( (mod->info[0]).index_data, LEN_32, "%.2f", power_watt[i][RAPL_PKG] );
							snprintf ( (mod->info[1]).index_data, LEN_32, "%.2f", power_watt[i][RAPL_PP0] );
							snprintf ( (mod->info[2]).index_data, LEN_32, "%.2f", power_watt[i][RAPL_PP1] );
							snprintf ( (mod->info[3]).index_data, LEN_32, "%.2f", power_watt[i][RAPL_DRAM] );
...
mod_register(struct module* mod)
{
	...
	register_module_fields(mod, rapl_mod_info, \
						  RAPL_MODULE_COL_NUM, rapl_start, rapl_read);
	return 0;
}
