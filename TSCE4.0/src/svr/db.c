...
RECONNECT:
    ...
    while (1) {
		...
		/* buffer for sql */
	char **sql_buffer = (char **) malloc(sizeof(char *) * n);
        for (k = 0; k < n; k++) {
		sql_buffer[k] = (char *) malloc((MAX_SQL_LEN_FOR_ONE_NODE * \
        		count + MAX_INSERT_SQL_PREFIX_LEN));
		if (sql_buffer[k] == NULL) {
		...
		/* produce sql prefix */
	int *sql_prefix_len = (int *)malloc(sizeof(int) * n);
	for (x = 0; x < n; x++) {
	    produce_insert_sql_prefix(mod_info_head, table_name,sql_buffer[x]);
	    sql_prefix_len[x] = strlen(sql_buffer[x]);
	}
        ...
        for (x = 0; x < n; x++) {
            ...
            ret = convert_data_to_sql(&g_data_buffer, \
                    datetime, sql_buffer[x] + sql_prefix_len[x]);
        ...
        for (x = 0; x < n; x++) {
            /* BLOCK PROBLEM */
            ret = mysql_query(con, sql_buffer[x]);
            if (ret != 0) {
                reconnect = 1;
        ...
        const config_mod_info_t *g_mod_info)
{
    ...
    const char * create_sql_suffix_4 = "create index node_name on DataInfo (node_name)";
    ...
    mysql_query(con, create_sql_suffix_3);
    mysql_query(con, create_sql_suffix_4);
/*
    if(ret != 0) {
